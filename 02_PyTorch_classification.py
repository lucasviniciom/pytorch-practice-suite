...
# Improving a model - by hyperparameters, values we can change on the model itself
...
# First try:
# More hidden units, number of layers, number of epochs
...
## Still not working because is missing:
## Non-linearity
...
# Evaluate the model trainder with non-linear activation functions
# Predictions:
model_3.eval()
with torch.inference_mode():
